...
// func (l *Ledger) GetAccountTransferHistoryByTime(accountId string, startTime, endTime int64) []*Transaction {
// 	output := make([]*Transaction, 0)
// 	account := l.Accounts[accountId]

// 	for _, txID := range account.History {
// 		transaction := l.Transactions[txID]
...
// func (l *Ledger) isUnderDailyTransferLimitByAccount(account string, amount, timestamp int64) bool {
// 	if l.Accounts[account].TransferLimitPerDay == 0 {
// 		return true
// 	}

// 	dailyTransfer := l.GetAccountTransferHistoryByTime(account, timestamp-86400000, timestamp)
// 	dailySummarizedTransfer := summarizeTotalTransfer(dailyTransfer) + amount
// 	return dailySummarizedTransfer <= l.Accounts[account].TransferLimitPerDay
// }
...
// 		if !l.isUnderDailyTransferLimitByAccount(fromAccount, amount, timestamp) {
// 			return false
// 		}
...
// 			if !l.isUnderDailyTransferLimitByAccount(t.FromAccount, t.Amount, t.Timestamp) {
...
// func (l *Ledger) Set24HourLimit(accountID string, limit int64) bool {
// 	if limit <= 0 {
// 		return false
// 	}
...
// 	account.TransferLimitPerDay = limit
// 	return true
// }

// func (l *Ledger) GetTopTransmitters(startTime int64, endTime int64, n int) []string {
// 	type kv struct {
// 		Account string
// 		Amount  int64
// 	}

// 	totalByTimePeriod := make([]kv, 0, len(l.Accounts))

// 	for k := range l.Accounts {
// 		transfers := l.GetAccountTransferHistoryByTime(k, startTime, endTime)
// 		total := summarizeTotalTransfer(transfers)
// 		if total > 0 {
// 			totalByTimePeriod = append(totalByTimePeriod, kv{Account: k, Amount: total})
// 		}
// 	}

// 	sort.Slice(totalByTimePeriod, func(i, j int) bool {
// 		if totalByTimePeriod[i].Amount != totalByTimePeriod[j].Amount {
// 			return totalByTimePeriod[i].Amount > totalByTimePeriod[j].Amount
// 		}
// 		return totalByTimePeriod[i].Account < totalByTimePeriod[j].Account
// 	})

// 	res := make([]string, min(n, len(totalByTimePeriod)))
// 	for i := 0; i < min(n, len(totalByTimePeriod)); i++ {
// 		res[i] = totalByTimePeriod[i].Account
// 	}

// 	return res
// }

// func summarizeTotalTransfer(trans []*Transaction) int64 {
// 	total := int64(0)
// 	for _, t := range trans {
...
// 	const oneDayMs int64 = 86400000

// 	// Test 1: Set 24h limit
// 	assert(l.Set24HourLimit("alice", 300) == true, "Test 1.1")
// 	assert(l.Set24HourLimit("unknown", 300) == false, "Test 1.2: Unknown account")
// 	assert(l.Set24HourLimit("alice", 0) == false, "Test 1.3: Limit <= 0")

// 	// Test 2: Transfer within limit
// 	// Alice transfers 200 at t = 100,000,000 (Remaining limit: 100)
// 	t1 := int64(100000000)
// 	assert(l.Transfer("tx_l1", "alice", "bob", 200, t1) == true, "Test 2.1")

// 	// Alice tries to transfer 150 at t1 + 1hr (exceeds 300 -> should fail)
// 	t2 := t1 + 3600000
// 	assert(l.Transfer("tx_l2", "alice", "bob", 150, t2) == false, "Test 2.2: Exceeds 24h limit")
// 	balA, _ := l.GetBalance("alice")
// 	assert(balA == 800, "Test 2.3: Alice balance remains 800")

// 	// Test 3: Rolling window sliding past 24 hours
// 	// Alice transfers 150 at t1 + 24h + 1ms (tx_l1 has rolled out of the 24h window!)
// 	t3 := t1 + oneDayMs + 1
// 	assert(l.Transfer("tx_l3", "alice", "bob", 150, t3) == true, "Test 3.1: Old tx rolled out of window")

// 	// Test 4: Dynamic Limit with Scheduled Transfers
// 	// Bob has a limit of 200
// 	l.Set24HourLimit("bob", 200)
// 	// Bob schedules tx at t3: 150 (valid)
// 	assert(l.ScheduleTransfer("tx_b_sched1", "bob", "charlie", 150, t3) == true, "Test 4.1")
// 	// Bob schedules tx at t3 + 10: 100 (will exceed 200 limit when processed together)
// 	assert(l.ScheduleTransfer("tx_b_sched2", "bob", "charlie", 100, t3+10) == true, "Test 4.2")

// 	// Process at t3 + 20: tx_b_sched1 succeeds (Bob spent 150), tx_b_sched2 fails (150+100 > 200)
// 	successCount := l.ProcessScheduledTransfers(t3 + 20)
// 	assert(successCount == 1, "Test 4.3: Only 1 should succeed due to limit")

// 	// Test 5: Top Transmitters Analytics
// 	// Transactions in system:
// 	// Alice: tx_l1 (200 @ t1), tx_l3 (150 @ t3)
// 	// Bob: tx_b_sched1 (150 @ t3)
// 	// Charlie: 0

// 	// Interval covering only t1: Only Alice (200)
// 	top := l.GetTopTransmitters(t1-10, t1+10, 5)
// 	assert(len(top) == 1 && top[0] == "alice", "Test 5.1: Alice only")

// 	// Interval covering both t1 and t3:
// 	// Alice = 200 + 150 = 350
// 	// Bob = 150
// 	top = l.GetTopTransmitters(t1-10, t3+10, 5)
// 	assert(len(top) == 2, "Test 5.2: 2 senders found")
// 	assert(top[0] == "alice" && top[1] == "bob", "Test 5.3: Alice (350) > Bob (150)")

// 	// Top with n = 1
// 	top1 := l.GetTopTransmitters(t1-10, t3+10, 1)
// 	assert(len(top1) == 1 && top1[0] == "alice", "Test 5.4: Top 1 is alice")

// 	// Tie-breaking test:
// 	// Deposit to charlie and make charlie send 150 at t3 to match Bob (150)
// 	l.Deposit("charlie", 500)
// 	assert(l.Transfer("tx_c1", "charlie", "alice", 150, t3) == true, "Test 5.5")

// 	// Interval covering t3 only:
// 	// Alice = 150, Bob = 150, Charlie = 150
// 	// All tie at 150 -> alphabetical order: "alice", "bob", "charlie"
// 	topTie := l.GetTopTransmitters(t3-10, t3+10, 3)
// 	assert(len(topTie) == 3, "Test 5.6: 3-way tie")
// 	assert(topTie[0] == "alice" && topTie[1] == "bob" && topTie[2] == "charlie", "Test 5.7: Tie-breaker order")

// 	// Non-existent or empty result
// 	topEmpty := l.GetTopTransmitters(0, 100, 5)
// 	assert(len(topEmpty) == 0, "Test 5.8: No transactions")
